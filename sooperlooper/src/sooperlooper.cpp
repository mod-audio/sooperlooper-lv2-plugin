...
enum {IN_0, OUT_0, OUT_WAVEFORM, PLAY_PAUSE, RECORD, RESET, UNDO, REDO, DRY_LEVEL, WET_LEVEL, WAVEFORM, OUT_STATUS, OUT_LOOP_POS, OUT_LOOP_LEN, PLUGIN_PORT_COUNT};

#define NUM_CHANNELS 1
#define PLUGIN_AUDIO_PORT_COUNT     2
#define PLUGIN_CONTROL_PORT_COUNT   PLUGIN_PORT_COUNT - PLUGIN_AUDIO_PORT_COUNT - 1
...
public:
    ...
    unsigned numSamplesPerPoint, overdubWaveformTurns;
...
{
    if (plugin->numSamplesPerPoint == 0 || plugin->overdubWaveformTurns == 0)
        return;
    ...
    if (++plugin->peakrec.count == plugin->numSamplesPerPoint)
    {
        ...
        if (plugin->sendWaveformRec && ++plugin->peakrec.dubcount == plugin->overdubWaveformTurns)
        {
            plugin->peakrec.dubcount = 0;
            plugin->sendWaveformRec = false;
            sendRecordWaveform(plugin);
        }
    }
...
void SooperLooperPlugin::run(LV2_Handle instance, uint32_t SampleCount)
{
    ...
    if (plugin->started == 0 && plugin->peak.used != 0) {
        ...
        plugin->numSamplesPerPoint = plugin->overdubWaveformTurns = 0;
        plugin->sendWaveformRec = false;
                            ...
                            lSampleIndex++)
                    {
                        for (unsigned c = 0; c < NUM_CHANNELS; c++) {
                            ...
                            *(loop->pLoopStart + lCurrPos) = fInputSample;
                            updateRecordPeak(plugin, lCurrPos, fInputSample);
                    ...
                    if (loop &&  loop->lLoopLength && loop->srcloop)
                    {
                            ...
                            lSampleIndex++)
                        {
                            for (unsigned c = 0; c < NUM_CHANNELS; c++) {
                                ...
                                fInputSample = pfInput[lSampleIndex];
                                updateOverdubPeak(plugin, lCurrPos, fInputSample);
                                                  ...
                                                  const void* data)
{
    ...
    plugin->overdubWaveformTurns = WAVEFORM_POINTS / plugin->numSamplesPerPoint;
    if (plugin->overdubWaveformTurns == 0)
        plugin->overdubWaveformTurns = 1;
